...
class PermisosModel extends Model {
    protected $table = 'permisos_perfil'; // Nombre según tu BD [cite: 33]
    ...
    public function getPermisosUsuario($idPerfil) {
        return $this->db->table('permisos_perfil as p')
            ->select('m.strNombreModulo, m.id as idModulo, p.bitConsulta, p.bitAgregar, p.bitEditar, p.bitEliminar, p.bitDetalle')
            ->join('modulo as m', 'm.id = p.idModulo') // [cite: 31, 35]
            ->where('p.idPerfil', $idPerfil)
            ->where('(p.bitConsulta = 1 OR p.bitAgregar = 1 OR p.bitEditar = 1 OR p.bitEliminar = 1 OR p.bitDetalle = 1)') // Solo si tiene acciones 
            .get()->getResultArray();
    }
}